# ===========================================
# Sticker Generator (Final Auto-Setup Version)
# ===========================================
# Features:
# ✅ Auto installs required packages
# ✅ Auto height (37mm/39mm)
# ✅ Smart autofit text scaling (Header + Desc + Footer + Remarks)
# ✅ Conditional footer (MAKE only if HEAT NUMBER exists)
# ✅ Optional logo + QR
# ✅ Safe save handling (no crash)
...
# -----------------------------
required = ["pandas", "openpyxl", "reportlab", "qrcode[pil]", "requests"]
for pkg in required:
    try:
...
def generate_stickers():
    ...
    for _, row in df.iterrows():
        ...
        # ================================
        # Full Auto-fit (including remarks)
        # ================================
        # Find starting font size that fits
        ...
        for font_size in [x / 10 for x in range(int(max_size * 10), int(min_size * 10) - 1, -1)]:
            ...
            total_height = (
                (len(header_lines) + len(wrapped_desc) + len(footer_fixed) + len(wrapped_remarks))
                * font_size * SPACING
            )
